...
func (c *Cx1Client) GetQueriesByLevelID_v310(level, levelId string) ([]AuditQuery_v310, error) {
	...
	switch level {
	case c.AuditQueryLevels_v310().TENANT:
		url = "/cx-audit/queries"
	case c.AuditQueryLevels_v310().PROJECT:
		url = fmt.Sprintf("/cx-audit/queries?projectId=%v", levelId)
	default:
		return queries_v310, fmt.Errorf("invalid level %v, options are currently: %v or %v", level, c.AuditQueryLevels_v310().TENANT, c.AuditQueryLevels_v310().PROJECT)
	}
	...
	for id := range queries_v310 {
		...
		case c.AuditQueryLevels_v310().TENANT:
			queries_v310[id].LevelID = c.AuditQueryLevels_v310().TENANT
		case c.AuditQueryLevels_v310().PROJECT:
			queries_v310[id].LevelID = levelId
		case c.AuditQueryLevels_v310().APPLICATION:
			if applicationId == "" {
		...
		case c.AuditQueryLevels_v310().PRODUCT:
			queries_v310[id].LevelID = c.AuditQueryLevels_v310().PRODUCT
		case "Tenant":
			queries_v310[id].LevelID = c.AuditQueryLevels_v310().TENANT
		}
	}
...
func (c *Cx1Client) AuditNewQuery_v310(language, group, name string) (AuditQuery_v310, error) {
	...
	newQuery, err := c.GetQueryByName_v310(c.AuditQueryLevels_v310().TENANT, c.AuditQueryLevels_v310().TENANT, language, "CxDefaultQueryGroup", "CxDefaultQuery")
	if err != nil {
		return newQuery, err
	}
...
func (c *Cx1Client) UpdateQueries_v310(level, levelid string, queries []QueryUpdate_v310) error {
	...
	if err != nil {
		...
		} else if level == c.AuditQueryLevels_v310().APPLICATION {
...
func (c *Cx1Client) GetQueries_v310() (SASTQueryCollection, error) {
	...
	aq, err := c.GetQueriesByLevelID_v310(c.AuditQueryLevels_v310().TENANT, "")
	if err != nil {
		return qc, err
	}
